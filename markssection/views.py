...
def exam_type_form(request):
    ...
    if request.method == "POST":
        ...
        ExamType.objects.create(minMarks=min_marks, maxMarks=max_marks,examName=Exam.objects.get(examName=exam_name), examType=exam_type, priority=priority)
...
def report_card(request):
    ...
    if request.method == "POST":
        ...
        if pk and class_room:
            ...
            exam_mapping = ExamMapping.objects.all()
            ...
            if "excel" in request.FILES:
                ...
                with NamedTemporaryFile(delete=False) as tmp:
                    ...
                    try:
                        ...
                        import win32api
                        import pythoncom
                        excel = client.DispatchEx("Excel.Application",pythoncom.CoInitialize())
                        excel.Visible = 0

                        wb = excel.Workbooks.Open(tmp.name)
                        #ws = wb.Worksheets[1]
                        try:
                            wb.SaveAs('c:\\tergetfolder\\record.pdf', FileFormat=57)
                        except:
                        ...
                        print ("Failed to convertttttttttttttttttttttt")
    ...
    return render(request, 'marks/reportCard.html',{"class_rooms":classrooms})
...
def class_report_analysis(request):
    ...
    if request.method == "GET":
        ...
        if "class_room" in request.GET:
            ...
            fail_students = 0
            didnt_appear = 0
            for ut_mark in ut_marks:
                if ut_mark.marks >= 33:
                    pass_students += 1
                elif ut_mark.marks < 33:
                    fail_students += 1
                else:
                    didnt_appear += 1
           ...
           # marks = marks.filter(
            #    examName__examName='Half-Yearly') | marks.filter(examName__examName='Annual')
            marks=marks.filter(examName__examName='UT-1') | marks.filter(examName__examName='UT-2')
...
def student_marks_filter(request):
    ...
    if exam_name != '' and exam_name is not None:
        addmission_number = user_profile.addmission_number
        student = ClassRoomStudent.objects.get(student__admissionNumber=addmission_number)
        marks = Marks.objects.filter(examName__examName=(exam_name),classroomStudent=student)
    if exam_type != '' and exam_type is not None:
        addmission_number = user_profile.addmission_number
        student = ClassRoomStudent.objects.get(student__admissionNumber=addmission_number)
        marks = Marks.objects.filter(examType__examType=(exam_type),classroomStudent=student)
    if subject != '' and subject is not None:
        addmission_number = user_profile.addmission_number
        student = ClassRoomStudent.objects.get(student__admissionNumber=addmission_number)
        marks = Marks.objects.filter(subject=subject,classroomStudent=student)
