...
def vehicle_list_excel(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="vehicle.xls"'
    response.write(dataset.xls)

    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet('Vahicle Data') # this will make a sheet named Users Data
    ...
    columns = ['vehicle_no', 'device_id', 'service_id', 'vehicle_type', 'no_of_seat','maximum_allowed','contact_person','insurance_date','permit_valid_date','pollution_cert_exp_date','service_date','fitness_service_date']
    ...
    rows = Vehicle.objects.all().values_list('vehicle_no', 'device_id', 'service_id', 'vehicle_type', 'no_of_seat','maximum_allowed','contact_person','insurance_date','permit_valid_date','pollution_cert_exp_date','service_date','fitness_service_date')
    for row in rows:
        row_num += 1
