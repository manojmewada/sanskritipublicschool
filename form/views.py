...
def form(request):
    ...
    if request.method == "POST":
        ...
        user_profile = UserProfile.objects.create(
            user=user, fullName=f_name + " " + l_name, addmission_number=add_number, user_type="Student", password=phone_number, image=documents.photo)
...
def upload_excel_data(request):
    if request.method == "POST":
        ...
        #wb = copy(rb)
        permanent_sheet = wb['Sheet2']
        current_sheet = wb['Sheet3']
        i = 2
  ...
  #          class_section = request.GET["classSection"]
    #        students = students.filter(classSection__icontains=class_section)
        if "addNumber" in request.GET:
            ...
            students = students.filter(admissionNumber__icontains=(add_no))
            if students:
                student_info = StudentInfo.objects.get(admissionNumber__icontains=(add_no))
                documents = Documents.objects.get(student=student_info)
                p_add = PermanentAddress.objects.get(student=student_info)
                c_add = CurrentAddress.objects.get(student=student_info)
                parent = ParentInfo.objects.get(student=student_info)
                route = StudentRoute.objects.get(student=student_info)
                return render(request, 'form/student_details.html',  {"student": student_info, "pAdd": p_add, "parent": parent, "route":route,"documents":documents,"cAdd": c_add,"students": students, "values": request.GET, "class_rooms": ClassRoom.objects.all()})
            else:
                
                messages.error(
                    request, 'Cant find student with entered detail')
                return redirect('recordForm')
    return render(request, 'form/searchPage2.html', {"class_rooms": ClassRoom.objects.all()})
